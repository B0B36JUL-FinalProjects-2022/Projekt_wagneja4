@testset "Simple KnapsackILP 1" begin
    ...
    @constraint(reference_model, green <= 1);
    @constraint(reference_model, blue <= 1);
    @constraint(reference_model, orange <= 1);
    @constraint(reference_model, yellow <= 1);
    @constraint(reference_model, gray <= 1);
